...
void pass1()
{
    ...
    file.open("testinput.txt",ios::in);
    ...
    while(end_count!=1)
    {
        ...
        else if(isop(lip))//if isop not a label
        {
            temp.op=optab(lip);
            temp.loc=ten_to_hex(loc_num);
            loc_num+=3;
        ...
        else
        {
            ...
            else if(lip=="BYTE"||lip=="RESW"||lip=="RESB"||lip=="WORD")//If instruction is a byte resw resb...
            {
                temp.op=lip;
                if(lip=="WORD")
                {
...
void record()
{
    ...
    out<<"H^"<<line[0].label<<"  00"<<line[0].parameter<<"^"<<length<<endl;
    ...
    for(int i=1,insnum=0,j=1;i<line.size()-1;i++)
    {
        if(insnum==0)
        {
            out<<"T^00"<<line[i].loc<<"^"<<ten_to_hex(l[j])<<"^"<<line[i].obcode;;
            j++;
            insnum++;
        }
        else if(insnum==10)
